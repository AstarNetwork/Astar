...
describe('runtime upgrade', async () => {
    ...
    it('runtime upgrade works', async () => {
        ...
        const codePath = path.join(__dirname, `../../../target/release/wbuild/${runtime}-runtime/${runtime}_runtime.compact.compressed.wasm`)
        const code = readFileSync(codePath);
        await api.tx.sudo.sudoUncheckedWeight(api.tx.system.setCode('0x' + code.toString('hex')), 0).signAndSend(alice);
