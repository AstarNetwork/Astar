...
pub struct OvmExecutable<T: Config> {
...
impl<T: Config> Loader<T> for PredicateLoader {
...
#[derive(Clone)]
pub struct ExecutionContext<T: Config> {
    pub self_account: T::AccountId,
    pub depth: usize,
    // pub deferred: Vec<DeferredAction<T>>,
    pub config: Rc<OvmConfig>,
...
pub struct PredicateOvm<T: Config> {
    _schedule: Rc<Schedule>,
    _phantom: PhantomData<T>,
}

impl<T: Config> PredicateOvm<T> {
...
impl<T: Config> Vm<T> for PredicateOvm<T> {
    type Executable = OvmExecutable<T>;
