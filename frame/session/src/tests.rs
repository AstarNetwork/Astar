...
use mock::{
    ...
    Origin, PreUpgradeMockSessionKeys, Session, System, SESSION_CHANGED, TEST_SESSION_CHANGED,
};
...
fn upgrade_keys() {
    ...
    assert_eq!(
        mock::NEXT_VALIDATORS.with(|l| l.borrow().clone()),
        vec![1, 2, 3]
    );
    ...
    new_test_ext().execute_with(|| {
        ...
        let mock_keys_for = |val| mock::MockSessionKeys {
            dummy: UintAuthorityId(val),
        };
