...
decl_storage! {
    ...
    add_extra_genesis {
        ...
        build(|config: &GenesisConfig<T>| {
            ...
            <Module<T>>::genesis_init_keys(&config.keys);
        });
    }
}
...
impl<T: Config> Module<T> {
    pub fn genesis_init_keys(keys: &Vec<(T::AccountId, T::ValidatorId, T::Keys)>) {
        ...
        let queued_keys: Vec<_> = initial_validators_1
            .iter()
            .cloned()
            .map(|v| (v.clone(), <Module<T>>::load_keys(&v).unwrap_or_default()))
            .collect();
