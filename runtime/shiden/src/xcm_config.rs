...
use super::{
    ...
    RuntimeOrigin, ShidenAssetLocationIdConverter, TransactionLengthFeeFactor, TreasuryAccountId,
    XcAssetConfig, XcmWeightToFee, XcmpQueue, MILLISDN,
};
use crate::weights;
use frame_support::{
    parameter_types,
...
impl SafeCallFilter {
    ...
    pub fn allow_base_call(call: &RuntimeCall) -> bool {
        match call {
            ...
            RuntimeCall::PolkadotXcm(call) => !matches!(
                call,
                pallet_xcm::Call::send { .. } | pallet_xcm::Call::execute { .. }
            ),
            _ => false,
        }
    }
...
parameter_types! {
    ...
    pub FeeAssetId: xcm::latest::AssetId = xcm::latest::AssetId(ShidenLocation::get());
    /// Flat fee charged per outbound XCMP message, on top of the per-byte fee.
    pub const BaseDeliveryFee: Balance = MILLISDN;
}
