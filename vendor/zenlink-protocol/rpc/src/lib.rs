...
pub trait ZenlinkProtocolApi<BlockHash, AccountId> {
	...
	fn get_all_pairs(&self, at: Option<BlockHash>) -> Result<Vec<PairInfo<AccountId, NumberOrHex>>>;

	#[rpc(name = "zenlinkProtocol_getOwnerPairs")]
	fn get_owner_pairs(&self, owner: AccountId, at: Option<BlockHash>)
	...
	C::Api: ZenlinkProtocolRuntimeApi<Block, AccountId>,
{
	...
	fn get_all_pairs(&self, at: Option<<Block as BlockT>::Hash>) -> Result<Vec<PairInfo<AccountId, NumberOrHex>>> {
		let api = self.client.runtime_api();
		let at = BlockId::hash(at.unwrap_or_else(|| self.client.info().best_hash));
