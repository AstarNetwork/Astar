...
use pallet_xcm_transactor::{
    chain_extension::Error as XcmCEError, OnCallback, QueryConfig, QueryType,
};
use parity_scale_codec::{Decode, Encode};
use sp_runtime::traits::Bounded;
use xcm::{prelude::*, v3::Response};
use xcm_builder::test_utils::OnResponse;
...
fn xcm_remote_contract_callback() {
    ...
    const SELECTOR_CONSTRUCTOR: [u8; 4] = [0xC0, 0xDE, 0xCF, 0xFF];
    const SELECTOR_XCM_FLIP: [u8; 4] = [0xC0, 0xDE, 0xCA, 0xFE];
    const SELECTOR_GET: [u8; 4] = [0xC0, 0xDE, 0xCE, 0xEE];
    const SELECTOR_TEST: [u8; 4] = [0xC0, 0xDE, 0xC0, 0x00];
