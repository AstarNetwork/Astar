...
use crate::{Balances, Grandpa, MaximumBlockWeight, NegativeImbalance, System};
...
mod tests {
    ...
    fn empty_chain_simulation() {
        ...
        loop {
            ...
            wm = next;
            if wm == WeightMultiplier::from_rational(-1, 1) {
                break;
            }
            iterations += 1;
        }
        println!(
            "iteration {}, new wm = {:?}. Weight fee is now zero",
            iterations, wm
        );
    }
    ...
    fn weight_to_fee_should_not_overflow_on_large_weights() {
        ...
        vec![t + 100, t * 2, t * 4].into_iter().for_each(|i| {
            let fm = WeightMultiplierUpdateHandler::convert((i, max_fm));
