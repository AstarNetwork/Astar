...
fn shibuya_session_keys(aura: AuraId) -> SessionKeys {
    SessionKeys { aura }
}

fn shiden_session_keys(aura: AuraId) -> shiden_runtime::SessionKeys {
    shiden_runtime::SessionKeys { aura }
}

/// Gen Shibuya chain specification for given parachain id.
pub fn get_shiden_chain_spec(para_id: u32) -> ShidenChainSpec {
    // Alice as default
    ...
    ShidenChainSpec::from_genesis(
        ...
        "shiden",
        ChainType::Live,
        move || make_shiden_genesis(endowned.clone(), sudo_key.clone(), para_id.into()),
        vec![],
        None,
        None,
        None,
...
/// Gen Shibuya chain specification for given parachain id.
pub fn get_shibuya_chain_spec(para_id: u32) -> ShibuyaChainSpec {
    // Alice as default
    let sudo_key = get_account_id_from_seed::<sr25519::Public>("Alice");
    let endowned = vec![
        (
    ...
    ShibuyaChainSpec::from_genesis(
        ...
        "shibuya",
        ChainType::Live,
        move || make_shibuya_genesis(endowned.clone(), sudo_key.clone(), para_id.into()),
        vec![],
        None,
        None,
        None,
...
fn make_shibuya_genesis(
    ...
    GenesisConfig {
        ...
        session: SessionConfig {
            keys: authorities
                .iter()
                .map(|x| (x.0.clone(), x.0.clone(), shibuya_session_keys(x.1.clone())))
                .collect::<Vec<_>>(),
        },
        aura: AuraConfig {
            authorities: vec![],
...
fn make_shiden_genesis(
    ...
    shiden_runtime::GenesisConfig {
        ...
        session: shiden_runtime::SessionConfig {
            keys: authorities
                .iter()
                .map(|x| (x.0.clone(), x.0.clone(), shiden_session_keys(x.1.clone())))
                .collect::<Vec<_>>(),
        },
