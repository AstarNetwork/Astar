    ...
    async fn create_inherent_data_providers(
        ...
        let (relay_parent_storage_root, relay_chain_state, relay_parent_descendants) =
            if self.client.runtime_api()
                ...
                .unwrap_or(false)
            {
                let offset = self.client.runtime_api()
                    .relay_parent_offset(parent)
                    .expect("API present, call should succeed; qed");
                RelayStateSproofBuilder::default()
                    .into_state_root_proof_and_descendants(offset.into())
            } else {
                let (root, proof) = RelayStateSproofBuilder::default()
