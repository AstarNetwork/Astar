...
impl SubstrateCli for Cli {
    ...
    fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
        load_spec(id, self.run.parachain_id.unwrap_or(2000))
    }
    ...
    match &cli.subcommand {
        ...
        None => {
            ...
            runner.run_node_until_exit(|config| async move {
				...
				let chain_spec_para_id = extension.map(|e| e.para_id);

                // Evaluation order
                // para id passed by arg -> para id in chainspec
                let id = ParaId::from(match (cli_arg_para_id, chain_spec_para_id) {
                    (Some(cli_arg_para_id), Some(chain_spec_para_id)) => {
                        if cli_arg_para_id != chain_spec_para_id {
                            warn!("specified parachain id doesn't match to the one defined in chainspec");
                        }
                        cli_arg_para_id
                    },
