...
use plasm_runtime::constants::currency::TDAT;
use plasm_runtime::Block;
use plasm_runtime::{
    AuraConfig, BalancesConfig, ContractsConfig, EVMConfig, EthereumConfig, GenesisConfig,
    GrandpaConfig, IndicesConfig, PlasmLockdropConfig, SudoConfig, SystemConfig, WASM_BINARY
};
...
use plasm_runtime::NodeAuthorizationConfig; // The genesis config that serves for our pallet.
use bs58::decode;
...
fn testnet_genesis(
    ...
    keys: Vec<(AccountId, AuraId, GrandpaId)>,
    endowed_accounts: Option<Vec<AccountId>>,
    sudo_key: AccountId,
) -> GenesisConfig {
    const ENDOWMENT: Balance = 1_000_000_000 * TDAT;
    ...
    make_genesis(wasm_binary, initial_authorities, keys, endowed_accounts, sudo_key, true)
}
...
fn make_genesis(
    ...
    GenesisConfig {
        ...
        pallet_node_authorization: Some(NodeAuthorizationConfig {
			nodes: vec![
			(
    OpaquePeerId(bs58::decode("12D3KooWGcHwjUBgazrUQ2paCzt6Jc2XN9BUsKXVAPZjVNxrftfi").into_vec().unwrap()),
