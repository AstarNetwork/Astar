    ...
    match matches.subcommand() {
        ...
        ("inspect", Some(matches)) => {
            let uri = match matches.value_of("uri") {
                Some(uri) => uri.into(),
                None => {
                    rpassword::read_password_from_tty(Some("URI: ")).expect("Failed to read URI")
                }
            };
    ...
    <T as FromStr>::Err: std::fmt::Debug,
{
    ...
    str::parse::<T>(str_value)
        .unwrap_or_else(|_| panic!("Invalid `{}' parameter; expecting an integer.", name))
}
