//! Contracts that expressed balances conform to the ERC20.
//!
//! This is a contract that expressed balances.
//! It conforms to the ERC20.
//! We can handle assets on Plasma using a contract that conforms to ERC20.
//! Specifically, the contract must be implemented `transfer_from` and` transfer` functions.
...
//            });
        ...
        pub(external) fn total_supply(&self) -> Balance {
            ...
            env.println(&format!("Balances::total_supply = {:?}", total_supply));
            total_supply
        }
        ...
        pub(external) fn balance_of(&self, owner: AccountId) -> Balance {
            ...
            env.println(&format!("Balances::balance_of(owner = {:?}) = {:?}", owner, balance));
            balance
        }
        ...
        pub(external) fn allowance(&self, owner: AccountId, spender: AccountId) -> Balance {
            ...
            env.println(&format!(
                "Balances::allowance(owner = {:?}, spender = {:?}) = {:?}",
